...
Reply createScheduleTask::SchedulePress(semanticAnalysisTask &semanticTask)
{
    ...
    if (createJsonData == nullptr) {
        qCWarning(PluginLogger) << "Failed to cast json data to CreateJsonData";
        return errorMessage();
    }
    Reply m_reply;
    if (createJsonData->getPropertyStatus() != JsonData::PRO_NONE
            || createJsonData->offset() > -1) {
        qCWarning(PluginLogger) << "Invalid property status or offset - Status:" << createJsonData->getPropertyStatus() 
                               << "Offset:" << createJsonData->offset();
        REPLY_ONLY_TTS(m_reply, CREATE_ERR_TTS, CREATE_ERR_TTS, true);
        return m_reply;
    }
    //如果时间无效
    if (createJsonData->getDateTimeInvalid()) {
        qCWarning(PluginLogger) << "Invalid date time in create request";
    ...
    if (m_begintime > m_endtime) {
        //日程的开始时间大于结束时间，
        qCritical(PluginLogger) << "Invalid schedule time range - Begin:" << m_begintime << "End:" << m_endtime;
    }
    if (shouldEndSession(createJsonData)) {
        if (!isValidDateTime) {
            qCInfo(PluginLogger) << "Invalid schedule time - Begin:" << m_begintime << "End:" << m_endtime;
            //无效时间，直接返回回复语
            ...
            if (beginDateTimeBeforeCurrent()) {
                qCInfo(PluginLogger) << "Schedule begin time is before current time - Begin:" << m_begintime;
                ...
                qCInfo(PluginLogger) << "Schedule begin time is beyond half year - Begin:" << m_begintime;
                //"只能创建未来半年的日程"
            ...
            qCInfo(PluginLogger) << "Created schedule with ID:" << scheduleID;
        ...
        isValidDateTime = true;
        qCDebug(PluginLogger) << "Starting multi-round schedule creation";
...
void createScheduleTask::setDateTime(CreateJsonData *createJsonData)
{
    ...
    case 2: {
        ...
        qCDebug(PluginLogger) << "Two time points - First has time:" << firstScheduleHasTime 
                             << "Second has time:" << secondScheduleHasTime;
        //有两个时间
        m_begintime = createJsonData->getDateTime().suggestDatetime.at(0).datetime;
        ...
        if (!validDateTime(m_begintime)) {
            QString strDateTime = createJsonData->getDateTime().suggestDatetime.at(0).strDateTime;
            qCWarning(PluginLogger) << "Invalid begin date time:" << strDateTime;
            isValidDateTime = false;
            replyNotValidDT = QString(ISVALID_DATE_TIME).arg(strDateTime);
            break;
        }
        ...
        if (!validDateTime(m_endtime)) {
            ...
            qCWarning(PluginLogger) << "Invalid end date time:" << endDateTime;
            isValidDateTime = false;
        ...
        if (m_begintime.date() == QDateTime::currentDateTime().date()) {
            if (m_begintime.time() > QTime::currentTime()) {
                ...
                qCDebug(PluginLogger) << "Adjusting begin time for same day schedule";
                m_begintime.setDate(m_begintime.date().addDays(m_begintime.date().daysTo(QDate::currentDate())));
            } else {
                //跨天日程，如果日程开始时间小于等于当前时间，则开始时间加一天
                qCDebug(PluginLogger) << "Adjusting begin time to next day";
        ...
        if (firstScheduleHasTime && !secondScheduleHasTime) {
            qCDebug(PluginLogger) << "Setting end time to match begin time";
            m_endtime.setTime(m_begintime.time());
        } else if (!firstScheduleHasTime && secondScheduleHasTime) {
            qCDebug(PluginLogger) << "Setting begin time to match end time";
            m_begintime.setTime(m_endtime.time());
        }
    } break;
    default: {
        //其他情况视为错误输入
        qCWarning(PluginLogger) << "Invalid number of date time suggestions:" << DateTimeSize;
        isValidDateTime = false;
...
{
    qCDebug(PluginLogger) << "Creating schedule with repeat status:" << createJsonData->getRepeatStatus();
    //创建的日程信息
    QString scheduleID;
    //重复日程的时间
    ...
    case CreateJsonData::NONE: {
        ...
        if (m_begintime >= QDateTime::currentDateTime() && m_begintime < QDateTime::currentDateTime().addMonths(6)) {
            qCDebug(PluginLogger) << "Creating non-repeating schedule";
    ...
    case CreateJsonData::EVED:
        //每天重复日程
        qCDebug(PluginLogger) << "Creating daily repeating schedule";
        scheduleID = getEveryDaySchedule();
        break;
    case CreateJsonData::EVEW: {
        //每周重复日程
        qCDebug(PluginLogger) << "Creating weekly repeating schedule - Days:" << getDayNum;
        scheduleID = getEveryWeekSchedule(getDayNum);
    }
    break;
    case CreateJsonData::EVEM: {
        //每月重复日程
        qCDebug(PluginLogger) << "Creating monthly repeating schedule - Days:" << getDayNum;
    ...
    case CreateJsonData::EVEY: {
        ...
        if (m_begintime > QDateTime::currentDateTime() && m_begintime < QDateTime::currentDateTime().addMonths(6)) {
            qCDebug(PluginLogger) << "Creating yearly repeating schedule";
    ...
    case CreateJsonData::WORKD:
        //工作日
        qCDebug(PluginLogger) << "Creating workday repeating schedule";
        scheduleID = getEveryWorkDaySchedule();
        break;
    case CreateJsonData::RESTD: {
        //休息日
        qCDebug(PluginLogger) << "Creating rest day repeating schedule";
        scheduleID = getEveryRestDaySchedule();
    }
    break;
    }
    qCDebug(PluginLogger) << "Created schedule with ID:" << scheduleID;
    return scheduleID;
}
...
{
    qCDebug(PluginLogger) << "Checking if begin time is within half year - Begin:" << m_begintime;
    //判断日程开始时间是否在半年内
    if (m_begintime < QDateTime::currentDateTime() || m_begintime > QDateTime::currentDateTime().addMonths(6)) {
        qCDebug(PluginLogger) << "Begin time is outside half year range";
        return false;
    }
    qCDebug(PluginLogger) << "Begin time is within half year range";
    return true;
}

bool createScheduleTask::beginDateTimeOutHalfYear()
{
    qCDebug(PluginLogger) << "Checking if begin time is beyond half year - Begin:" << m_begintime;
    //日程开始时间不在半年范围内
    if (m_begintime > QDateTime::currentDateTime().addMonths(6)) {
        qCDebug(PluginLogger) << "Begin time is beyond half year";
        return true;
    }
    return false;
}
...
{
    qCDebug(PluginLogger) << "Checking if begin time is before current time - Begin:" << m_begintime;
    //日程开始时间早于当前时间
    if (m_begintime < QDateTime::currentDateTime()) {
        qCDebug(PluginLogger) << "Begin time is before current time";
        return true;
    }
    return false;
}
...
{
    qCDebug(PluginLogger) << "Creating non-repeating schedule - Begin:" << m_begintime << "End:" << m_endtime;
    //设置重复类型
    m_widget->setRpeat(0);
    //创建日程
...
{
    qCDebug(PluginLogger) << "Creating daily repeating schedule - Begin:" << m_begintime << "End:" << m_endtime;
    //设置重复类型
    m_widget->setRpeat(1);
    DSchedule::Ptr scheduleinfo = setDateTimeAndGetSchedule(m_begintime, m_endtime);
...
{
    qCDebug(PluginLogger) << "Setting schedule date time and getting schedule - Begin:" << beginDateTime << "End:" << endDateTime;
